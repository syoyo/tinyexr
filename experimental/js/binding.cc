 ...
 public:
  ...
  /// `binary` is the buffer for EXR binary(e.g. buffer read by fs.readFileSync)
    ...
    error_.clear();
		...
		result_ = LoadEXRFromMemory(&rgba, &width_, &height_, reinterpret_cast<const unsigned char *>(binary.data()), binary.size(), &err);
  ...
  // Return as memory views
    ...
    return emscripten::val(emscripten::typed_memory_view(image_.size(), image_.data()));
	}
