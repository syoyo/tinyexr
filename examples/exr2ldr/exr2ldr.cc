...
{
  int i = static_cast<int>(255.0f * powf(f, 1.0f / 2.2f)); // simple gamma correction.
  if (i > 255) i = 255;
  if (i < 0) i = 0;
...
bool SaveImage(const char* filename, const float* rgba, float scale, int width, int height) {

  std::vector<unsigned char> dst(width * height * 4);

  // alpha channel is also affected by `scale` parameter.
  for (size_t i = 0; i < width * height * 4; i++) {
    dst[i] = ftouc(rgba[i] * scale);
  }
...
{
  if (argc < 3) {
    printf("Usage: exr2ldr input.exr output.png (scale) (resize_factor).\n");
  ...
  bool ok = SaveImage(argv[2], &buf.at(0), scale, dst_width, dst_height);
